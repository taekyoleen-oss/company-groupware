...
- **관리자(approver / manager)**: role='manager'. 본인이 결재자로 지정된 직원의 휴가/취소만 결재. `/approvals` 진입.
- **실무자(member)**: role='member'. 일반 직원.

판정 헬퍼: `lib/auth/roles.ts` — `isSuperAdmin(profile)`, `isApprover(profile)`.
